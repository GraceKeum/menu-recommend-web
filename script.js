...
async function createRoom() {
  ...
  try {
    ...
    localStorage.setItem("currentRoomCode", inviteCode); // 입장할 때 찾기 위함
    go("room.html");
  } catch (e) {
...
// [변경] 7. 초대코드로 진짜 Firestore 방 찾아서 참여하기
async function joinRoom() {
  ...
  try {
    // rooms 컬렉션에서 대문자 변환한 초대코드가 일치하는 방 검색
    ...
    // 내 프로필을 members 배열에 추가(중복 방지는 UI나 로직으로 추가 가능)
...
async function showRoom() {
  ...
  try {
    ...
    // 추천 시스템 연산을 위해 로컬에 현재 멤버 상태 세이브
...
// 외부 HTML에서 함수를 실행할 수 있도록 전역(window) 설정 등록
